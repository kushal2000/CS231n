...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        out = gamma*y+beta
        flag = bn_param.get('flag', False)
        if not flag:
          running_mean = momentum*running_mean + (1-momentum)*sample_mean
          running_var = momentum*running_var + (1-momentum)*sample_var

        cache = {'x' : x, 'mean': sample_mean, 'var': sample_var, 'y': y, 'gamma': gamma, 'eps': eps}
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    x, mean, var, gamma, y, eps = cache['x'], cache['mean'], cache['var'], cache['gamma'], cache['y'], cache['eps']
    m = dout.shape[0]
    dy = dout*gamma
    dvar = -0.5*np.sum(dy*(x-mean), axis = 0)*((var+eps)**(-1.5))
    dmean = -np.sum(dy/(np.sqrt(var+eps)), axis = 0) - 2*dvar*np.sum(x-mean, axis = 0)*(1/m)
    dx = dy*(1/(np.sqrt(var+eps))) + dvar*2*(x-mean)/m + dmean/m
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout*y, axis = 0)
    dbeta = np.sum(dout, axis=0)
    dx = (dout*gamma - np.sum(dout*gamma, axis = 0)/N - np.sum(dout*gamma*y, axis = 0)*y/N)/np.sqrt(var+eps)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    w_new = int(1 + (W+2*p-ww)/s)
    out = np.zeros((n, f, h_new, w_new))
    for i in range(h_new):
      for j in range(w_new):
        for k in range(f):
          for t in range(n):
            out[t, k, i, j] = np.sum(x[t, :, i*s: i*s+hh, j*s: j*s+ww]*w[k]) + b[k]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(h_new):
      for j in range(w_new):
        for k in range(f):
          for t in range(n):
            dx[t, :, i*s: i*s+hh, j*s: j*s+ww] += dout[t,k,i,j]*w[k]
            dw[k] += x[t, :, i*s: i*s+hh, j*s: j*s+ww]*dout[t,k,i,j]
            db[k] += dout[t,k,i,j]
            # out[t, k, i, j] = np.sum(x[t, :, i*s: i*s+hh, j*s: j*s+ww]*w[k]) + b[k]
    dx = dx[:, :, p:-p, p:-p]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(h_new):
      for j in range(w_new):
        for k in range(c):
          for t in range(n):
            out[t, k, i, j] = np.max(x[t, k, i*s: i*s+p_h, j*s: j*s+p_w])
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(h_new):
      for j in range(w_new):
        for k in range(c):
          for t in range(n):
            temp = x[t, k, i*s: i*s+p_h, j*s: j*s+p_w]
            DX, DY = np.unravel_index(np.argmax(temp, axis=None), temp.shape)
            dx[t, k, i*s+DX, j*s+DY] += dout[t, k, i, j]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_g = x.reshape(x.shape[0]*G, -1)  # N*G,H*W*(C/G)
